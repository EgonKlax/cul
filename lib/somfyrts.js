...
module.exports.cmd = function (shuttertext, address, command, configFile) {
    ...
    // check addree
    if (!address.match(/^[a-fA-F\d]{6}$/)) {
        return false;
    }

    // get rolling code
    rollingCode = getNextRollingCode(configFile, address, shuttertext, command);
...
function getNextRollingCode(configFile, deviceAddress, shuttertext, command) {
    let propertyFound;
    ...
    if (propertyFound === undefined && typeof propertyFound == 'undefined') {
        ...
        propertyFound["lastCommand"] = command;
        propertyFound["lastUpdateOn"] = new Date();
